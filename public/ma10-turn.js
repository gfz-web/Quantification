...
function calculateTurnMa(rows, period = strategy.period) {
...
function maZone(row) {
  if (!row || row.slope === null) {
    return '暂无数据';
  }
  if (row.slope > 0) {
    return `${strategy.lineLabel}向上`;
  }
  if (row.slope < 0) {
    return `${strategy.lineLabel}向下`;
  }
  return `${strategy.lineLabel}走平`;
}
  ...
  return {
    ...
    tooltip: {
      ...
      formatter(params) {
        ...
        return [
          ...
          `MA${period} 日变化: ${formatNumber(row.slope)}`,
    ...
    toolbox: {
      ...
      feature: {
        ...
        restore: {},
        saveAsImage: { name: `${meta.symbol}-ma${period}-turn-${START_DATE}` }
      },
...
function renderSwitcher(defaultSymbol, payload) {
  ...
  Object.entries(payload).forEach(([symbol, indexData]) => {
    ...
    button.innerHTML = `
      ...
      <span>MA${strategy.period} ${formatNumber(latest.ma)} / ${maZone(latest)}</span>
    `;
...
function showIndex(symbol) {
  ...
  const validRows = rows.filter((row) => row.slope !== null);
  const latest = validRows[validRows.length - 1];
  const meta = selected.meta;

  if (!rows.length || !latest) {
    document.getElementById('error').hidden = false;
    document.getElementById('error').textContent = `当前日期范围没有可用的${strategy.lineLabel}拐头数据。`;
    return;
  }

  const color = maColor(latest);
  const progress = Math.max(0, Math.min(100, latest.slope * 10 + 50));

  document.getElementById('error').hidden = true;
  setText('page-title', `${meta.name}${strategy.lineLabel}拐头`);
  setText('badge-symbol', `Tencent 数据源 / ${meta.name} / MA${strategy.period} 拐头`);
  setText('intro-text', `按自定义日期区间展示${meta.name}${strategy.lineLabel}拐头策略。规则为 MA${strategy.period} 斜率由非正转正买入，由非负转负卖出。`);
  setText('ma-slope', formatNumber(latest.slope));
  ...
  setText('chart-heading', `2025 年以来 ${meta.name} ${strategy.lineLabel}拐头走势`);
  setText('chart-subtitle', `展示${meta.name}收盘价、MA${strategy.period} 和拐头买卖点；支持拖动缩放、悬停查看每日明细。`);
  setText('legend-price', `${meta.name}收盘价`);
  document.title = `${meta.name} ${strategy.lineLabel}拐头策略`;
...
async function loadIndex(symbol, name) {
  ...
  const maRows = calculateTurnMa(rows, strategy.period);
  const validRows = maRows.filter((row) => row.slope !== null);
  const latest = validRows[validRows.length - 1];

  if (!latest) {
    throw new Error(`${name} 没有可用的${strategy.lineLabel}拐头数据。`);
  }
...
async function initInternal() {
  ...
  try {
    ...
    if (!defaultSymbol) {
      throw new Error(warnings.join('; ') || `没有可用的${strategy.lineLabel}拐头数据。`);
    }
...
function initTurnMaStrategy(config = {}) {
  strategy = {
...
export { init, initTurnMaStrategy };
