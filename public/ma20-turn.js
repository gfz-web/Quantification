  ...
  return {
    ...
    tooltip: {
      ...
      formatter(params) {
        ...
        return [
          ...
          `MA20 日变化: ${formatNumber(row.slope)}`,
    ...
    toolbox: {
      ...
      feature: {
        ...
        restore: {},
        saveAsImage: { name: `${meta.symbol}-ma20-turn-${START_DATE}` }
      },
...
function renderSwitcher(defaultSymbol, payload) {
  ...
  Object.entries(payload).forEach(([symbol, indexData]) => {
    ...
    button.innerHTML = `
      ...
      <span>MA20 ${formatNumber(latest.ma)} / ${maZone(latest)}</span>
    `;
...
function showIndex(symbol) {
  ...
  if (!rows.length || !latest) {
    ...
    document.getElementById('error').textContent = '当前日期范围没有可用的二十日线拐头数据。';
    return;
  }
  ...
  setText('page-title', `${meta.name}二十日线拐头`);
  setText('badge-symbol', `Tencent 数据源 / ${meta.name} / MA20 拐头`);
  setText('intro-text', `按自定义日期区间展示${meta.name}二十日线拐头策略。规则为 MA20 斜率由非正转正买入，由非负转负卖出。`);
  ...
  setText('chart-heading', `2025 年以来 ${meta.name} 二十日线拐头走势`);
  setText('chart-subtitle', `展示${meta.name}收盘价、MA20 和拐头买卖点；支持拖动缩放、悬停查看每日明细。`);
  setText('legend-price', `${meta.name}收盘价`);
  document.title = `${meta.name} 二十日线拐头策略`;

  renderTradeTable(rows);
  renderBacktestSummary(rows);
  renderChart(rows, MA_PERIOD, meta);
}
...
async function loadIndex(symbol, name) {
  ...
  if (!latest) {
    throw new Error(`${name} 没有可用的二十日线拐头数据。`);
  }
...
async function init() {
  ...
  try {
    ...
    if (!defaultSymbol) {
      throw new Error(warnings.join('; ') || '没有可用的二十日线拐头数据。');
    }
